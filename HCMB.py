...
def plotheatmap(D_mat_real, mapName, method ,title=""):
    ...
    mpl.rcParams.update({'font.size': 10})  # Font-size
    fig, ax = plt.subplots()
    ...
    cb = fig.colorbar(m)
    ax.set_title(title)
    plt.savefig('heatmap_{0}_{1}.png'.format(mapName, method), dpi=300)
    plt.show()
...
def check_result_plot(D_mat_real, mapName, method):
    ...
    fig.suptitle('{0} Normalized Map on {1}'.format(method,mapName), fontsize=18)

    ax1 = fig.add_subplot(2, 1, 1)  # Axes first plot
    ax1.set_title('Sum along row/column')  # Title first plot
    ax1.set_xlabel('Position')  # X-label


    ax3 = fig.add_subplot(2, 1, 2)  # Axes third plot
    ax3.set_title('Variance along row/column')
    ax3.set_xlabel('Position')


    ax1.plot(r_sum, marker='o', lw=0, ms=2,c='r')  # Plot in first axes
   
    ax3.plot(r_var, marker='o', lw=0, ms=2,c='r')  # Plot in third axes


    ax1.get_yaxis().get_major_formatter().set_useOffset(False) 
    ax3.get_yaxis().get_major_formatter().set_useOffset(False)
    
    plt.savefig('checkSum_{0}_{1}.png'.format(mapName,method), dpi=300)
    plt.show()
...
def plot_compare(matrix1, matrix2, mapName, titleX, titleY):
    ...
    fig = plt.figure(figsize=(6, 6))  # Figure size
    fig.subplots_adjust(hspace=0.25, wspace=0.25)  # Space between sub-plots
    ...
    M1 = np.ma.masked_equal(matrix1, 0.0, copy=False)
    M2 = np.ma.masked_equal(matrix2, 0.0, copy=False)

    ax = fig.add_subplot(1, 1, 1)  # Axes plot
    ...
    # Plot only those bins that do not have nonzeros data
    # 散点图
    ax.scatter(M1.compressed(), M2.compressed(), marker='o', s=6,c='r')
