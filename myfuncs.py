...
"""
The function LM() is our main code of Hi-C Matrix Balancing (HCMB), 
based on iterative solution of equations, combining with linear search and projection strategy 
to normalize the hi-C original interaction data.
"""
...
def check_result_plot(D_mat_real, mapName):
    ...
    fig.suptitle('{}'.format(mapName), fontsize=18)
    ...
    plt.savefig('checkSum_{}.png'.format(mapName), dpi=300)
    plt.show()
...
def gen_stepsize_PG(x_current, A, dk, sigma, beta):
    assert sigma < 0.5, "Armijo rule is applicable for sigma less than 0.5"
    assert beta < 1, "Decay factor has to be less than 1"
...
#the main code
def LM(A, xk=None, kmax=10000, eps=1e-5, tmin=1e-16, verbose=False):
...
def get_norm_LM(matrix, addDiagonal=True):
    ...
    # Calculate normalization vector using LM
    a1=time.perf_counter()
    rmserror, x_solve, msg = LM(A, xk=x0, verbose=True)
...
def NormalizeKnightRuizV2(matrix, tol=1e-12, x0=None, delta=0.1, Delta=3, fl=0, addDiagonal=False):
...
def plotheatmap(D_mat_real, mapName, title=""):
    fig, ax = plt.subplots()
    ...
    #ax.set_title("LM Normalized contact counts", fontweight="bold")
    ax.set_title(title, fontweight="bold")
    plt.savefig('heatmap_{}.png'.format(mapName), dpi=300)
    plt.show()
...
def plot_comparison_for_chromosome(matrix1, matrix2, mapName, titleX, titleY):
    ...
    #    ax.set_xlabel('LM Normalized values')
    #    ax.set_ylabel('KR Normalized values')
    ax.set_xlabel(titleX)
    ax.set_ylabel(titleY)
    plt.savefig('compare_{}.png'.format(mapName), dpi=300)
    plt.show()
